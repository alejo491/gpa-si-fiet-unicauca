    ...
    function loadTeaching(idD, route) {
        ...
        $.getJSON(route, { term: idD }, function (data) {
            $.each(data, function (key, val) {
                ...
                if (val['social'] == "-1") { social = "-" } else if (val['social'] == "0") { social = "NC" } else { social = val['social'] }
                if (val['gestion'] == "-1") { gestion = "-" } else if (val['gestion'] == "0") { gestion = "NC" } else { gestion = val['gestion'] }
                if (val['investigacion'] == "-1") { investigacion = "-" } else if (val['investigacion'] == "0") { investigacion = "NC" } else { investigacion = val['investigacion'] }
                if (val['dProfesoral'] == "-1") { dProfesoral = "-" } else if (val['dProfesoral'] == "0") { dProfesoral = "NC" } else { dProfesoral = val['dProfesoral'] }
                if (val['docencia'] == "-1") { docencia = "-" } else if (val['docencia'] == "0") { docencia = "NC" } else { docencia = val['docencia'] }
