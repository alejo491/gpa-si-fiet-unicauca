    ...
    $.getJSON(route, { term: idF }, function (data) {
        ...
        $.each(data, function (key, val) {
            $("#DepartamentFrom" + idF + " tbody").append("<tr>" +
                    "<td>" + val['label'] + "</td>" +
                    "<td><a class='edit' href='/sedoc/AdminFac/VerDocentes/" + val['id'] + "'>Ver Docentes</a> </td>" +
                    "<td><a class='edit' href='/sedoc/AdminFac/VerLabores/" + val['id'] + "'>Ver Labores</a> </td>" +
                    "</tr>");
        });
    });
}
...
function loadTable(idFacultad) {
    ...
    if (idFacultad != 0) {
        ...
        $("#work-area").html("<table id='DepartamentFrom" + idfac + "' class='list' style='width:100%'>" +
                            ...
                            "<tbody></tbody>" +
                            "</table><br/>");
        loadDepartamentos(idFacultad, "/sedoc/AdminFac/GetFacultadPrograma/");
    }
}
