    ...
    $.getJSON(route, { term: idS, depa: idD }, function (data) {
        ...
        $.each(data, function (key, val) {
            $("#Materias" + idS + " tbody").append("<tr>" +
                              ...
                              "<td><a class='edit' href='/sedoc/AdminFac/ShowScoresMateria?depa=" + idD + "&tipo=" + val['id'] + "'>Evaluaci&oacute;n</a></td>" +
                              "<td><a class='edit' href='/sedoc/AdminFac/ShowAutoScoresMateria?depa="+ idD+"&tipo=" + val['id'] + "'>Autoevaluaci&oacute;n</a></td>" +
                              "</tr>");
        });
    });
}
...
function loadTable(idSemestre, idD) {
    ...
    if (idSemestre != 0) {
        $("#work-area").html("<table id='Materias" + ids + "' class='list' style='width:100%'>" +
                            ...
                            "<tbody></tbody>" +
                            "</table><br/>");
        loadMaterias(idSemestre, idD, "/sedoc/AdminFac/GetMateriasPrograma/");
    }
}
